...
async function sendClaudeRequest(request, response) {
    ...
    try {
        ...
        const useThinking = isSonnet5 || /^claude-(3-7|opus-4|sonnet-4|haiku-4-5|opus-4-5|opus-4-6|sonnet-4-6|opus-4-7)/.test(request.body.model);
        ...
        const isAdaptiveModel = isSonnet5 || /^claude-(opus-4-7|opus-4-8)/.test(request.body.model) || (enableAdaptiveThinking && /^claude-(opus-4-6|sonnet-4-6)/.test(request.body.model));
        const noSamplingModel = isSonnet5 || /^claude-(opus-4-7|opus-4-8)/.test(request.body.model);
        ...
        if (useThinking && isSonnet5 && reasoningEffort === 'auto') {
