...
async function sendClaudeRequest(request, response) {
    ...
    try {
        ...
        const isAdaptiveModel = isSonnet5 || /^claude-(opus-4-7)/.test(request.body.model) || (enableAdaptiveThinking && /^claude-(opus-4-6|sonnet-4-6)/.test(request.body.model));
        const noSamplingModel = isSonnet5 || /^claude-(opus-4-7)/.test(request.body.model);
